...
uint256 hashGenesisBlock("0x000006715c6857bbf3f68c8ef3ae9b36176bc44b3f6cf4e19de2774c23ba3a35");

static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // PennyBaseCoin: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "PennyBaseCoin Signed Message:\n";
...
{
    // PennyBaseCoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
// Launch specs
static const int64 nTargetTimespan = 3 * 60; // PennyBaseCoin: each block
static const int64 nTargetSpacing = 3 * 60; // PennyBaseCoin: 300 seconds
...
unsigned int static GetNextWorkRequired_V1(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // PennyBaseCoin: This fixes an issue where a 51% attack can change difficulty at will.
...
unsigned int static DarkGravityWave(const CBlockIndex* pindexLast, const CBlockHeader *pblock) {
    /* current difficulty formula, PennyBaseCoin - DarkGravity, written by Evan Duffield - evan@PennyBaseCoin.io */
    const CBlockIndex *BlockLastSolved = pindexLast;
    const CBlockIndex *BlockReading = pindexLast;
    const CBlockHeader *BlockCreating = pblock;
    BlockCreating = BlockCreating;
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // PennyBaseCoin: Special short-term limits to avoid 10,000 BDB lock limit:
...
{
    // PennyBaseCoin: temporarily disable v2 block lockin until we are ready for v2 transition
    return false;
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
      ...
      //vMerkleTree: 3f66108684400d84e2ffb528360d534fa19ecaf47c7c4c2ca1d1352ec4a351ad
        ...
        // Genesis block
        const char* pszTimestamp = "PennyBase";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        ...
        block.nVersion = 1;
        block.nTime    = 1501609637;
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 3191391;
        ...
        assert(block.hashMerkleRoot == uint256("0x2430827cfd45b68a3450fff9979d07941862c19f98967c644ee797db6b6b3317"));
        block.print();
...
unsigned char pchMessageStart[4] = { 0xa6, 0xd2, 0xc3, 0xf6 }; // PennyBaseCoin: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("PennyBaseCoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("PennyBaseCoinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static PennyBaseCoinMiner(CWallet *pwallet)
{
    printf("PennyBaseCoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("PennyBaseCoin-miner");
    ...
    try { while (true) {
        ...
        printf("Running PennyBaseCoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("PennyBaseCoinMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&PennyBaseCoinMiner, pwallet));
}
